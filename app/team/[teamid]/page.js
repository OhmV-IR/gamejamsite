...
export default function TeamPage({ params }) {
                ...
                fetch("/api/fetchbasicuserinfo", {
                    ...
                    res.json().then(body => {
                        setJoinRequests(prev => {
                            if (!prev.some(jr => jr.uid == viewerUid)) {
                                return [...prev, { uid: viewerUid, name: body.name, pfp: body.pfp }]
                            } else {
                                return prev;
                            }
                        });
        ...
        fetch("/api/addtoteam", {
            ...
            if (res.ok) {
                ...
                fetch("/api/fetchbasicuserinfo", {
                    ...
                    res.json().then(body => {
                        setMembers(prev => {
                            if (!prev.some(member => member.uid == uidToAdd)) {
                                return [...prev, { uid: uidToAdd, name: body.name, pfp: body.pfp }];
                            }
                            else {
                                return prev;
                            }
    ...
    function AcceptJoinRequest(uid) {
        fetch("/api/acceptjoinrequest", {
            ...
            if (res.ok) {
                ...
                setJoinRequests(joinRequests.filter(jr => jr.uid != uid));
                setTimeout(() => setOkBannerDisplay(false), 7000);
            }
            else {
                setFailedBannerDisplay(true);
    ...
    function KickFromTeam(uid) {
        if (uid == ownerId) {
            // HACK: importing boostrapjs breaks a whole bunch of stuff, so using an invisible button to trigger the modal.
            document.getElementById("dangerTeamDeleteModalKickBtn").click();
            document.getElementById("dangerModalKickConfirm").onclick = () => KickFromTeamUnsafe(uid, true);
        }
        else {
            KickFromTeamUnsafe(uid, false);
        }
    }
        ...
        fetch("/api/leaveteam", {
            ...
            if (res.ok) {
                if (isdelete) {
                    ...
                    setMembers(members.filter(member => member.uid != viewerUid));
                    setTimeout(() => setOkBannerDisplay(false), 7000);
                }
            }
            else {
    ...
    function KickFromTeamUnsafe(uid, isdelete) {
        fetch("/api/teamkick", {
            method: "POST",
            credentials: "include",
            body: JSON.stringify({
                            ...
                            fetch("/api/fetchbasicuserinfo", {
                                ...
                                if (res.ok) {
                                    res.json().then(memberbody => {
                                        setMembers(prev => {
                                            if (!prev.some(member => member.uid == body.members[i].uid)) {
                                                return [...prev, { uid: body.members[i].uid, name: memberbody.name, pfp: memberbody.pfp }];
                                            }
                                            else {
                                                return prev;
                                            }
                            ...
                            fetch("/api/fetchbasicuserinfo", {
                                ...
                                if (res.ok) {
                                    res.json().then(requesterbody => {
                                        ...
                                        setJoinRequests(prev => {
                                            if (!prev.some(jr => jr.uid == body.joinrequests[i].uid)) {
    ...
    return (
        <div>
            ...
            <div className={`w-100 ${styles.centeralign} ${styles.ownertext} mt-3`}>
                Owned by:&nbsp;&nbsp;
                <button className="btn" onClick={() => window.location.href = `/user/${ownerId}/`}>
            ...
            {(isAdmin || IsJamRunning()) && ownerId == viewerUid && submission.size == null && !isUploading
            ...
            {(isAdmin || IsJamRunning()) && ownerId == viewerUid && submission.state == 1 && !isUploading
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <div className="mb-3">
                                ...
                                <input type="text" className="form-control" name="uid" placeholder="123123-asasdas1321-asdjkasd" value={uidToAdd} onChange={evt => setUidToAdd(evt.target.value)}></input>
                            </div>
                        </div>
                        <div className="modal-footer">
                            <button className="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            ...
                            <tbody>
                                {
                                    members.map(member => (
                                        <tr key={member.uid}>
                                            <td>
                                                <button className="btn" onClick={() => window.location.href = `/user/${member.uid}`}>
                                                    <span className="avatar avatar-sm" style={{ backgroundImage: `url(${member.pfp})` }}></span>
                                                    &nbsp;&nbsp;
                                                    <span className={styles.membername}>{member.name}</span>
                                                </button>
                                                {isAdmin || ownerId == viewerUid
                                                    ? <button className={`btn btn-danger ${styles.lmbtn}`} onClick={() => KickFromTeam(member.uid)}><IconKarate></IconKarate>&nbsp;Kick</button>
                                                    : <></>
                                                }
                                                {(isAdmin || ownerId == viewerUid) && member.uid != ownerId
                                                            ...
                                                            <div className="modal-footer">
                                                                <div className="w-100">
                                                                    <div className="row">
                                                                        ...
                                                                        <div className="col">
                                                                            <button className="btn btn-danger w-100" data-bs-dismiss="modal" onClick={() => TransferOwnership(member.uid)}> Transfer Ownership </button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
            ...
            {joinRequests.length > 0 && (isAdmin || viewerUid == ownerId)
                ? <div className="row w-100 mt-5">
                    <div className="col">
                            ...
                            <tbody>
                                {
                                    joinRequests.map(request => (
                                        <tr key={request.uid}>
                                            <td>
                                                <button className="btn" onClick={() => window.location.href = `/user/${request.uid}`}>
                                                ...
                                                <button className={`btn btn-success ${styles.lmbtn}`}><IconCheck onClick={() => AcceptJoinRequest(request.uid)}></IconCheck></button>
                                                <button className={`btn btn-danger ${styles.lmbtn}`}><IconX onClick={() => RejectJoinRequest(request.uid)}></IconX></button>
                                            </td>
                                        </tr>
                                    ))
                                }
